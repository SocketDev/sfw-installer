...
function saveToDisk(res: IncomingMessage, fd: number, bytesAlreadyDownloaded: number, aborted: () => boolean): Promise<void> {
...
async function downloadWithResume(url: string, fd: number, bytesAlreadyDownloaded: number, aborted: () => boolean, hash: crypto.Hash, expectedDigestHex: string, pendingDownloadPath: string): Promise<IncomingMessage | null> {
...
export async function eagerAcquireLockAndDownload(url: string, destination: string, expectedDigestAlgo: string, expectedDigestHex: string) {
  ...
  try {
    $res = await downloadWithResume(url, fd, bytesAlreadyDownloaded, () => aborted, hash, expectedDigestHex, pendingDownloadPath)
  } finally {
    $res?.destroy();
    await release().catch(() => {});
    try {
